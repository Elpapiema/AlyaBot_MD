...
let handler = async (m, { conn, usedPrefix }) => {
    ...
    m.react('🤗');
    let str = `\`${name2}\` está dando un fuerte abrazo a \`${name}\``.trim();

    if (m.isGroup) {
        try {
            // Obtener el archivo JSON desde GitHub
            const response = await axios.get('https://raw.githubusercontent.com/Elpapiema/Adiciones-para-AlyaBot-RaphtaliaBot-/refs/heads/main/video_json/SFW/abrazar.json');
